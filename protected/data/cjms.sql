...
('0ddd9716-d65a-f32b-a965-03741999aad0', 'tableCName', '页面标题', 'tableCName', 'return ''<a href="javascript:void(0);" onclick="viewItem(\\'''' .$row[''tableId'']. ''\\'');"><span title="''.$row[''tableInfo''].''">'' . ((int)strlen($row[''tableCName'']) > 50 ? htmlspecialchars(substr($row[''tableCName''], 0, 60) . " ..."): htmlspecialchars($row[''tableCName'']))\n. ''</span></a>'';', 'ee524ea0-10e2-edf9-9ad5-7222c4b567cc', 'int', '', '1', '', 0, 3, '', '', 1, '', 1),
