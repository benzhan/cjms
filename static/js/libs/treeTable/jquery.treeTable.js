    ...
    $.fn.treeTable = function (opts) {
        ...
        $treeTable.click(function (event) {
            ...
            if ($target.attr('controller')) {
                ...
                //判断是否是叶子节点
                if (!className || className.indexOf(css['AN']) == -1 && className.indexOf(css['N']) == -1) { return; }
        ...
        function formatNode(tr) {
            ...
            if (cMap[id] == 0) {
                ...
                if (!$cur.attr('isFirstOne')) {
                    var $preSpan = $('#' + $cur.attr('prevId'), $treeTable).children("td,th").eq(opts.column).find('.prev_span').clone();
                } else {
                    var $parent = $('#' + cMap[id], $treeTable);
                    //没有上一个兄弟节点，则使用父节点的prev_span
                    var $preSpan = $parent.children("td,th").eq(opts.column).find('.prev_span').clone();
            ...
            var $td = $cur.children("td,th").eq(opts.column);
            $td.prepend('<span controller="true" arrow="true" class="' + className + '"></span>').prepend($preSpan);
        };
